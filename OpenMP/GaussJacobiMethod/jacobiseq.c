...
void test_solution(LinSys *linsys, data_t *solution) {
    ...
    do {
        ...
        if (choice >= N || choice < 0) {
            printf(
                "\tInvalid input. Equations are only indexed between 0 and %d. "
                "Try again!\n",
                N - 1);
        } else {
            valid = 1;
...
void check_flags(char *argv[], int argc) {
    if (argc - 1 == CLI_ARG_NUM) return;  // no flags
...
int main(int argc, char *argv[]) {
    ...
    if (!OMIT_PRINT) linsys_print(&linsys);
    ...
    if (!OMIT_TEST) test_solution(&linsys, res);
