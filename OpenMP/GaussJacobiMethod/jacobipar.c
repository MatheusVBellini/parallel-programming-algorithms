...
void gen_linear_system(LinSys *linsys) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        // filling up A
        data_t row_sum = 0;
        #pragma omp simd reduction(+ : row_sum) aligned(linsys : 32)
        for (int j = 0; j < N; j++) {
...
data_t *solve(LinSys *normsys, data_t *x, data_t e) {
    ...
    data_t *x_plus = (data_t *)aligned_malloc(sizeof(data_t) * N);
    data_t *x_minus = (data_t *)aligned_malloc(sizeof(data_t) * N);
    memcpy(x_minus, x, N * sizeof(data_t));

    #pragma omp parallel num_threads(T) shared(converged, lock)
    {
      while(!converged)
      {
        #pragma omp for simd aligned(x_plus, x : 32)
        for (int i = 0; i < N; i++)
            x_plus[i] = x[i];

        #pragma omp for simd aligned(x_plus, normsys : 32)
        for (int i = 0; i < N; i++)
            x_plus[i] = normsys->b[i];
        
        #pragma omp for reduction(+ : x_plus[ : N])
        for (int i = 0; i < N; i++) {
            // here we change steps
            for (int j = 0; j < N; j++) {
                x_plus[i] += normsys->A[i][j] * x[j];
            }

            // N = N + 1, N - 1 = N
            //problema nas duas linhas de baixo
            //x = x_plus;
            //x_minus = x;
            omp_unset_lock(&lock);
        }

        //ensures everyone finishes calculating x_plus before for the error 
        #pragma omp barrier
        ...
        #pragma omp single
        {
            #pragma omp task
            {
              ...
              // if error is smoller than e, exit while
              if (calc_err(x0, x1) <= e) converged = true;
              else
              {
                omp_set_lock(&lock);
                memcpy(x_minus, x, N * sizeof(data_t));
                memcpy(x, x_plus, N * sizeof(data_t));
                omp_unset_lock(&lock);
              }
                // while error is greater than e, keep calculating error with x_minus and x
                /*
                while (!flag) {
                    memcpy(x0, x_minus, N * sizeof(data_t));
                    memcpy(x1, x, N * sizeof(data_t));
                    if (calc_err(x0, x1) <= e) flag = 1;

                    //
                    omp_set_lock(&lock);
                    printf("erro normal %f e erro encontrado %f\n", e, calc_err(x0, x1));
                }
          ...
          #pragma omp taskwait    // waiting for the task to complete
        }
